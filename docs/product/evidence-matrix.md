...
| Config doctor/init | executable config validation and starter generation | `.krn/current/config-doctor.json`, `.krn/current/config-init-result.json` | CLI tests | `pnpm test`, `krn config doctor` | command policy can reject target-specific commands until profile is tuned | use in next target adoption run |
