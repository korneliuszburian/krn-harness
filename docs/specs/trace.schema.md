...
`install.ran` records status, created/skipped counts, optional reason, and compact action summaries with path/kind/status only. `hook.received` records provider, event, support status, result status, guardrail decision, `enforced: false`, payload source, detail, and finding codes. Warned or blocked hook decisions must include finding codes.
